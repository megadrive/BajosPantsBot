...
public class RaffleJoinCommand {
				...
				if (foundRole != null) {
					if (foundRole.getPositionRaw() < verifieduserRole.getPositionRaw()) {
						if (!user.getRoles().contains(foundRole)) {
							...
							returnMessage.setMessage("You've joined the `" + foundRole.getName() + "` raffle!");
							returnMessage.setColor(Color.GREEN);
						} else {
							returnMessage.setMessage(":broken_heart: You're already in that raffle...");
