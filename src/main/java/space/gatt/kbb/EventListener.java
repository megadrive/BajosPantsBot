...
public class EventListener extends ListenerAdapter {
	...
	public void onMessageReceived(MessageReceivedEvent event) {
		...
		if (event.getChannelType() == ChannelType.TEXT) {
			...
			if (!event.getMember().getRoles().contains(KingMain.getVerifiedMember()) && (
				...
				KingMain.getMsgMan().sendMessage(event.getMember(), Color.YELLOW,
						"Hey! I noticed you haven't verified " +
								"yourself yet! Head over to <#309932250114293761> and verify yourself to get access to cool " +

								"colours and custom roles!");
			}

			if (event.getMessage().getContent().toLowerCase().contains(
					"thank") && event.getMessage().getContent().toLowerCase().contains("the")
					&&
	...
	public void onGuildMemberRoleAdd(GuildMemberRoleAddEvent event) {
		System.out.println("Found add role event");
		int colours = 0;
		for (Role r : event.getMember().getRoles()){
			if (r.getName().toLowerCase().startsWith("c: ")){
				colours++;
			}
		}
		Role hoarder = event.getGuild().getRoleById("302806963396280320");
		if (colours > 2){
			KingMain.getMsgMan().sendMessage(event.getMember(), new ReturnMessage().setColor(hoarder.getColor())
					.setMessage("Oh no... You're... You're a ROLE HOARDER! This hurts... Remove some of those " +
							"colours that you don't want ;)"));
			event.getGuild().getController().addRolesToMember(event.getMember(), hoarder).complete();
			return;
		}
	}
	...
	public void onGuildMemberRoleRemove(GuildMemberRoleRemoveEvent event) {
		System.out.println("Found remove role event");
		int colours = 0;
		for (Role r : event.getMember().getRoles()){
			if (r.getName().toLowerCase().startsWith("c: ")){
				colours++;
		...
		Role hoarder = event.getGuild().getRoleById("302806963396280320");
		if (colours <= 2){
			if (event.getMember().getRoles().contains(hoarder)){
				event.getGuild().getController().removeRolesFromMember(event.getMember(), hoarder).complete();
				KingMain.getMsgMan().sendMessage(event.getMember(), new ReturnMessage().setColor(hoarder.getColor())
						.setMessage("Well done! You're not a Role Hoarder anymore <3"));
			}
		}
	}
	...
	public void onGuildMemberJoin(GuildMemberJoinEvent event) {
		KingMain.getMsgMan().log(KingMain.getDebugChannel(), Color.GREEN,
				event.getMember().getEffectiveName() + " has " +
						"joined the server!");
		KingMain.getMsgMan().sendMessage(event.getGuild().getTextChannelById("290285351099039747"), Color.GREEN,
				KingMain
						.getEmoteStorage().get(
						"omg").getAsMention() + " Welcome to the Bajo Community Discord, " + event.getMember()
						.getAsMention() + "!\n" +
						"Message any Mod-Mate your questions or queries.\n" +
						"Remember to read <#309672559237136385> and <#294443160581701632>! " + KingMain
						.getEmoteStorage().get(
						"thanks").getAsMention());
	}
